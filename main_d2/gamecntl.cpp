...
void transfer_energy_to_shield(fix time)
{
	...
	e = std::min(std::min(time * CONVERTER_RATE, Players[Player_num].energy - INITIAL_ENERGY), (MAX_SHIELDS - Players[Player_num].shields) * CONVERTER_SCALE);
...
//[ISB] wow watcom c was uh... special...
...
void ReadControls()
{
	int key;
	fix key_time;
	static uint8_t exploding_flag = 0;
